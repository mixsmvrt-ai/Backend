    ...
    for (const model of models) {
      ...
      for (let attempt = 0; attempt <= env.AI_QUALITY_RETRIES; attempt += 1) {
        ...
        try {
          ...
          const correction = qualityFeedback ? `\n\nPrevious attempt failed validation: ${qualityFeedback}\nReturn one complete replacement JSON object only. Do not return {}, [], markdown, prose, truncated strings, or explanatory text. Include every required field: tempo, key, scale, timeSignature, trackName, notes, chordProgression, structure, pluginRecommendations, and tracks when applicable. Keep arrays concise enough to finish the response within the output limit.` : "";
