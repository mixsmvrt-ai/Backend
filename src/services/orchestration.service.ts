...
export function shouldRetryGenerationAttempt(error: unknown, attempt: number, maxRetries: number) {
  ...
  if (error instanceof AiOrchestrationError && error.code === "AI_INVALID_RESPONSE") return false;
    ...
    for (const model of models) {
      ...
      for (let attempt = 0; attempt <= env.AI_QUALITY_RETRIES; attempt += 1) {
        ...
        try {
          ...
          const correction = qualityFeedback ? `\n\nPrevious attempt failed validation: ${qualityFeedback}\nReturn one complete replacement JSON object only. Do not return {}, [], markdown, prose, truncated strings, or explanatory text. Include every required field: tempo, key, scale, timeSignature, trackName, notes, chordProgression, structure, pluginRecommendations, and tracks when applicable. Keep the 8-bar response concise: use playable note arrays, short strings, and no unnecessary production prose so the JSON finishes completely.` : "";
          music = validateStructuredMusicQuality(await provider.compose(buildMusicPrompt(resolvedInput, `${musicBrain.providerPrompt}\n${referencePrompt}\nCurated MIDI reference feature profiles: ${referenceBlend.featureSummary}\n\nREFERENCE-FIRST RULE: use the PRIMARY reference event list as the compositional foundation. Reconstruct its note density, rests, pocket, phrase length, motif repetition, duration distribution, velocity contour, register, and role before applying controlled changes requested by the user. Preserve approximately 95% of its construction quality, but mutate selected pitches, phrase endings, voicings, octaves, durations, and motif responses so the result is new. Do not add notes to increase complexity; if the reference is sparse, remain sparse. Keep the requested musical role. ${referenceAttachments}${correction}\n\nFINAL OUTPUT OVERRIDE: return only the compact required JSON object. Omit tracks entirely. Return no more than 32 notes, 8 chordProgression strings, 4 structure entries, and 2 short pluginRecommendations. Use empty arrays for optional musical layers. Do not include productionNotes, variationSuggestions, explanations, or any extra keys. Close the JSON completely.`), controller.signal), input.lengthBars, input.kind);
          usedModel = model;
          break;
        } catch (error) {
          lastError = error;
