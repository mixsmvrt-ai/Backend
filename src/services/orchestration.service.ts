...
export async function orchestrateGeneration(userId: string, input: OrchestrationInput) {
  ...
  try {
    ...
    const { error: fileError } = await db.from("generation_files").insert({ generation_id: generation.id, user_id: userId, storage_path: storagePath, file_name: fileName, mime_type: "audio/midi", file_size_bytes: Buffer.isBuffer(file) ? file.length : file.toString().length }); if (fileError) throw fileError;
