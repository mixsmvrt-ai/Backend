...
export function buildCompactPlannerPrompt(context: BuiltAiContext, options: CompactPromptOptions) {
  ...
  const systemPrompt = [
    ...
    "Treat the curated MIDI reference DNA as the primary musical quality anchor. Stay close to its blended groove, rhythmic contour, note density, phrase lengths, register, tension, voicing color, repetition-to-variation balance, velocity shape, and human feel so the result belongs in the same professional MIDI pack, but create new notes, rhythms, motifs, hooks, and progressions without copying.",
