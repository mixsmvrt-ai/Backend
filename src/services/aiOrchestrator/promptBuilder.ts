...
export function buildCompactPlannerPrompt(context: BuiltAiContext, options: CompactPromptOptions) {
  ...
  const systemPrompt = [
    ...
    "Artist references are vibe descriptors only. Extract high-level characteristics and create a completely original composition; never reproduce a melody, hook, lyric, or copyrighted song.",
      ...
      "Composition priority is strict: curated MIDI reference DNA is the primary musical authority, the user's explicit request is second, and artist/genre profiles are a light flavor layer only. Stay close to the reference groove, rhythmic contour, note density, phrase lengths, register, tension, voicing color, repetition-to-variation balance, velocity shape, and human feel. Never let an artist or genre descriptor override the reference vocabulary.",
