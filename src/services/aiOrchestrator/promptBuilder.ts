...
export function buildCompactPlannerPrompt(context: BuiltAiContext, options: CompactPromptOptions) {
  ...
  const systemPrompt = [
    ...
    "Artist references are vibe descriptors only. Extract high-level characteristics and create a completely original composition; never reproduce a melody, hook, lyric, or copyrighted song.",
      "Artist references are secondary flavor descriptors only. Extract high-level characteristics and create a completely original composition; never reproduce a melody, hook, lyric, or copyrighted song.",
      "Composition priority is strict: curated MIDI reference DNA is the primary musical authority, the user's explicit request is second, and artist/genre profiles are a light flavor layer only. Aim for very strong reference-style similarity by closely matching the reference pocket, rhythmic contour, note density, phrase architecture, register balance, tension, voicing color, repetition-to-variation balance, velocity shape, arrangement roles, and human feel. Deliberately create a new motif, note order, interval path, phrase ending, voicing detail, and harmonic movement; never copy source note sequences, hooks, recognizable rhythms, or complete progressions.",
